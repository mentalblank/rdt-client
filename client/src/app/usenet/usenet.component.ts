...
export class UsenetComponent implements OnInit, OnDestroy {
  ...
  private usenetStatusSubscription: Subscription;

  public sortProperty = 'added';
  public sortDirection = 'desc';
  public tab: 'queue' | 'history' = 'queue';

  constructor(private usenetService: UsenetService, private torrentService: TorrentService) {}

  ngOnInit(): void {
    this.load();
    this.usenetStatusSubscription = this.torrentService.usenetStatus$.subscribe((status) => {
      this.activeConnections = status.active;
      this.maxConnections = status.max;
    });
  }

  ngOnDestroy(): void {
    if (this.usenetStatusSubscription) {
      this.usenetStatusSubscription.unsubscribe();
    }
  }
