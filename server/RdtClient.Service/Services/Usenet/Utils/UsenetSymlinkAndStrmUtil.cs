...
public static class UsenetSymlinkAndStrmUtil
{
    ...
    {
        const String command = "find . \\( -type l -o -name '*.strm' \\) -print0 | xargs -0 sh -c 'for path in \"$@\"; do echo \"$path\"; if [ \"${path##*.}\" = \"strm\" ]; then cat \"$path\"; else readlink \"$path\"; fi; done' sh";
    ...
    private static Boolean IsStrm(FileInfo x) =>
        x.Extension.Equals(".strm", StringComparison.CurrentCultureIgnoreCase);

    private static Boolean IsSymLink(FileInfo x) =>
