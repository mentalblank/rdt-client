...
public class UsenetQueueManager(DataContext dataContext, INntpClient usenetClient, UsenetSymlinkManager symlinkManager, ILogger<UsenetQueueManager> logger)
{
    ...
    public async Task<String> AddNzbFile(Byte[] bytes, String fileName, String? category, Int32 priority)
    {
        try
        {
            ...
            // Enforce global uniqueness for JobName to avoid WebDAV path collisions
            ...
            symlinkManager.CreateSymlinks(job);
    ...
    public async Task DeleteJob(String hash, Boolean deleteData)
    {
        ...
        if (job != null)
        {
            if (deleteData)
            {
                ...
                // Delete symlinks
                var settings = Settings.Get.Usenet;
                if (!String.IsNullOrWhiteSpace(settings.SymlinkPath))
                {
                    try
                    {
                        var jobPath = Path.Combine(settings.SymlinkPath, job.Category ?? "", job.JobName);
                        if (Directory.Exists(jobPath))
                        {
                            Directory.Delete(jobPath, true);
                            logger.LogInformation($"Deleted symlink directory: {jobPath}");
                            
                            // Try to delete category folder if empty
                            if (!String.IsNullOrWhiteSpace(job.Category))
                            {
                                var categoryPath = Path.Combine(settings.SymlinkPath, job.Category);
                    ...
                    {
                        logger.LogWarning($"Error deleting symlinks for job {job.JobName}: {ex.Message}");
                    }
                }
            }
            else
