...
public class UsenetNzbDocument
{
    ...
    public static async Task<UsenetNzbDocument> LoadAsync(Stream stream)
    {
        var document = new UsenetNzbDocument();
    ...
    private static async Task ReadSegmentsAsync(XmlReader reader, NzbFile file)
    {
        ...
        while (true)
        {
            ...
            if (reader is { NodeType: XmlNodeType.Element, Name: "segment" })
            {
                ...
                {
                    Bytes = Int64.TryParse(bytesAttr, out var bytes) ? bytes : 0,
