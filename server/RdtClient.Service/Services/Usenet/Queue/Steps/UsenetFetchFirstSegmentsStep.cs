...
public static class UsenetFetchFirstSegmentsStep
{
    ...
    )
    {
        ...
        var maxConnections = settings.MaxDownloadConnections > 0 ? settings.MaxDownloadConnections : 10;
        
        return await nzbFiles
            .Where(x => x.Segments.Count > 0)
            .Select(x => FetchFirstSegment(x, usenetClient, cancellationToken))
            .WithConcurrencyAsync(maxConnections + 5)
    ...
    public class NzbFileWithFirstSegment
    {
        ...
        public required Boolean MissingFirstSegment { get; init; }
        public required DateTimeOffset ReleaseDate { get; init; }

        public Boolean HasRar4Magic() => HasMagic(Rar4Magic);
        public Boolean HasRar5Magic() => HasMagic(Rar5Magic);

        private Boolean HasMagic(Byte[] sequence)
        {
