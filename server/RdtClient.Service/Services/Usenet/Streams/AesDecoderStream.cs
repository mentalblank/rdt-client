...
namespace RdtClient.Service.Services.Usenet.Streams
{
    internal sealed class AesDecoderStream : Stream
    {
        ...
        private Int32 _plainStart;
        private Int32 _plainEnd;
        private readonly Byte[] _cipherBuffer; // buffer to read ciphertext blocks in chunks

        private Int64 _mWritten; // number of decoded bytes returned already
        private readonly Int64 _mLimit;
        private Boolean _isDisposed;
        private Int64? _pendingSeekPosition = null;
        ...
        private const Int32 DefaultPlainBufferSize = 4 << 10; // 4096
        private const Int32 DefaultCipherBufferBlocks = 8; // read up to 8 blocks at once
        ...
        public AesDecoderStream(Stream input, AesParams aesParams)
        {
            ...
            if (((UInt32)input.Length & (BlockSize - 1)) != 0)
            {
        ...
        public override Int64 Length => _mLimit;
        ...
        public override async Task<Int32> ReadAsync(Byte[] buffer, Int32 offset, Int32 count, CancellationToken ct)
        {
            ...
            Int32 toReturnAllowed = (Int32)Math.Min(count, _mLimit - _mWritten);
            Int32 totalCopied = 0;

            // First, deliver any already-decrypted bytes in the plain buffer
            if (_plainEnd > _plainStart)
            {
                Int32 have = _plainEnd - _plainStart;
                Int32 take = Math.Min(have, toReturnAllowed);
            ...
            while (toReturnAllowed > 0 && _mWritten < _mLimit)
            {
                ...
                Int32 cipherRead = 0;
                Int32 firstRead = await _mStream.ReadAsync(_cipherBuffer, 0, _cipherBuffer.Length, ct)
                ...
                {
                    Int32 need = BlockSize - (cipherRead & (BlockSize - 1));
                    Int32 r = await _mStream.ReadAsync(_cipherBuffer, cipherRead, need, ct).ConfigureAwait(false);
                    if (r == 0)
                    {
                ...
                Int32 decryptOffset = 0;
                while (decryptOffset < cipherRead)
                {
                    Int32 cipherRemaining = cipherRead - decryptOffset;
                    Int32 plainSpace = _plainBuffer.Length - _plainEnd;
                    // If there is no plain space, compact the buffer if possible
                    if (plainSpace < BlockSize && _plainStart > 0)
                    {
                        Int32 existing = _plainEnd - _plainStart;
                        if (existing > 0)
                    ...
                    Int32 processBytes = Math.Min(cipherRemaining, plainSpace & ~(BlockSize - 1));
                    if (processBytes == 0)
                    {
                    ...
                    Int32 processed = _mDecoder.TransformBlock(_cipherBuffer, decryptOffset, processBytes, _plainBuffer,
                        _plainEnd);
        ...
        /// </summary>
        private async Task SeekInternalAsync(Int64 offset, CancellationToken ct)
        {
            if (offset < 0 || offset > _mLimit) throw new ArgumentOutOfRangeException(nameof(offset));

            Int64 blockIndex = offset / BlockSize;
            Int32 blockOffset = (Int32)(offset % BlockSize);

            Int64 cipherPosForRead = (blockIndex > 0) ? (blockIndex - 1) * BlockSize : 0L;
            ...
            if (blockIndex > 0)
            {
                ...
                {
                    Int32 r = await _mStream.ReadAsync(iv, read, BlockSize - read, ct).ConfigureAwait(false);
