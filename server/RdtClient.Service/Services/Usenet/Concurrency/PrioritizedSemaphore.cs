...
{
    private readonly LinkedList<TaskCompletionSource<Boolean>> _highPriorityWaiters = [];
    private readonly LinkedList<TaskCompletionSource<Boolean>> _lowPriorityWaiters = [];
    private SemaphorePriorityOdds _priorityOdds;
    private Int32 _maxAllowed;
    private Int32 _enteredCount;
    private Boolean _disposed = false;
    private readonly Lock _lock = new();
    private Int32 _accumulatedOdds;

    public PrioritizedSemaphore(Int32 initialAllowed, Int32 maxAllowed, SemaphorePriorityOdds? priorityOdds = null)
    {
    ...
    private static TaskCompletionSource<Boolean>? Release(LinkedList<TaskCompletionSource<Boolean>> queue)
    {
        while (queue.Count > 0)
        {
    ...
    public void UpdateMaxAllowed(Int32 newMaxAllowed)
    {
        lock (_lock)
        {
    ...
    public void Dispose()
    {
        List<TaskCompletionSource<Boolean>> waitersToCancel;
        lock (_lock)
        {
            if (_disposed) return;
            _disposed = true;
