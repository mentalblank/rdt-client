...
public class DebridLinkClient(ILogger<DebridLinkClient> logger, IHttpClientFactory httpClientFactory, IDownloadableFileFilter fileFilter) : ITorrentClient
{
    private DebridLinkFrNETClient GetClient()
    {
        ...
        {
            var apiKey = "";
            if (Settings.Get.Provider.Provider == Provider.DebridLink)
            {
                apiKey = Settings.Get.Provider.ApiKey;
            }
            else if (Settings.Get.SecondaryProvider.SecondaryProvider == SecondaryProvider.DebridLink) 
            {
                apiKey = Settings.Get.SecondaryProvider.SecondaryApiKey;
            }
            else
            {
                throw new InvalidOperationException("DebridLink is not set as a provider.");
            }
