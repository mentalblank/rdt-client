...
public class EnricherTest : IDisposable
{
    ...
    [Fact]
    public async Task EnrichMagnetLink_WhenTrackersAdded_PreservesQueryParams()
    {
        // Arrange
        var initialMagnetLink = "magnet:?xt=urn:btih:HASH123&dn=MyTestFile&tr=udp%3A%2F%2Ftracker.existing1";

        var newTrackersFromGrabber = new[]
        {
            "udp://new.tracker/announce", "udp://tracker.existing1"
        };
        ...
        // Act
        var result = await enricher.EnrichMagnetLink(initialMagnetLink);

        // Assert
        var resultUri = new Uri(result);
        var queryParams = System.Web.HttpUtility.ParseQueryString(resultUri.Query);

        // Verify standard parameters
        Assert.Equal("urn:btih:HASH123", queryParams["xt"]);
        Assert.Equal("MyTestFile", queryParams["dn"]);

        var trValues = queryParams.GetValues("tr")?.ToList() ?? new List<String>();
        Assert.Contains("udp://tracker.existing1", trValues);
        Assert.Contains("udp://new.tracker/announce", trValues);
        Assert.Equal(2, trValues.Count);
    }
    ...
    public async Task EnrichMagnetLink_WhenHashOnly_AddsTrackers(String magnetLink, String expectedResult, params String[] newTrackers)
    {
        // Arrange
        SetupTrackerListGrabber(newTrackers);
        var enricher = new Enricher(_loggerMock.Object, _trackerListGrabberMock.Object);
    ...
    [InlineData("magnet:?",
                   "magnet:?")]
    public async Task EnrichMagnetLink_WhenEndsWithQuestionMark_AddsTrackers(String magnetLink, String expectedResult, params String[] newTrackers)
    {
        // Arrange
        SetupTrackerListGrabber(newTrackers);
        var enricher = new Enricher(_loggerMock.Object, _trackerListGrabberMock.Object);
